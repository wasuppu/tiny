...
func castRay(orig, dir Vec3f, spheres []Sphere, lights []Light, depth int) Vec3f {
	...
	reflectOrig := point.Add(n.Muln(1e-3)) // offset the original point to avoid occlusion by the object itself
	if reflectDir.Dot(n) < 0 {
		reflectOrig = point.Sub(n.Muln(1e-3))
	}
	...
	for i := range len(lights) {
		...
		tmpMaterial := Material{albedo: Vec3f{1, 0, 0}}
	...
	return material.diffuseColor.Muln(diffuseLightIntensity).Muln(material.albedo[0]).Add(Vec3f{1, 1, 1}.Muln(specularLightIntensity).Muln(material.albedo[1])).Add(reflectColor.Muln(material.albedo[2]))
}
...
func main() {
	...
	mirror := Material{Vec3f{0.0, 10.0, 0.8}, Vec3f{1.0, 1.0, 1.0}, 1425}

	spheres := []Sphere{}
	spheres = append(spheres, Sphere{Vec3f{-3, 0, -16}, 2, ivory})
	spheres = append(spheres, Sphere{Vec3f{-1.0, -1.5, -12}, 2, mirror})
