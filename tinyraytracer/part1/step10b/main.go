...
type Shape interface {
	rayIntersect(orig, dir Vec3f) (bool, float64, Material, Vec3f, Vec3f)
}
...
func (s Sphere) rayIntersect(orig, dir Vec3f) (bool, float64, Material, Vec3f, Vec3f) {
	l := s.center.Sub(orig)
	tca := l.Dot(dir)
	d2 := l.Dot(l) - tca*tca
	...
	n := hit.Sub(s.center).Normalize()
	return true, t0, s.material, hit, n
}
...
func sceneIntersect(orig, dir Vec3f, shapes []Shape, material *Material, hit *Vec3f, n *Vec3f) bool {
	sphereDist := math.MaxFloat64
	for i := range shapes {
		isHit, disti, material1, hit1, n1 := shapes[i].rayIntersect(orig, dir)
		if isHit && disti < sphereDist {
			sphereDist = disti
			*material = material1
			*hit = hit1
			*n = n1
...
func castRay(orig, dir Vec3f, shapes []Shape, lights []Light, depth int) Vec3f {
	var point, n Vec3f
	material := NewMaterial()
	if depth > 4 || !sceneIntersect(orig, dir, shapes, &material, &point, &n) {
	...
	reflectColor := castRay(reflectOrig, reflectDir, shapes, lights, depth+1)
	refractColor := castRay(refractOrig, refractDir, shapes, lights, depth+1)
	...
	for i := range len(lights) {
		...
		if sceneIntersect(shadowOrig, lightDir, shapes, &tmpMaterial, &shadowPt, &shadowN) && shadowPt.Sub(shadowOrig).Length() < lightDistance {
			continue
		}
...
func render(shapes []Shape, lights []Light) {
	...
	for j := range height {
		for i := range width {
			...
			framebuffer[i+j*width] = toColor(castRay(Vec3f{0, 0, 0}, dir, shapes, lights, 0))
		}
	}
...
func main() {
	...
	shapes := []Shape{}
	shapes = append(shapes, Sphere{Vec3f{-3, 0, -16}, 2, ivory})
	shapes = append(shapes, Sphere{Vec3f{-1.0, -1.5, -12}, 2, glass})
	shapes = append(shapes, Sphere{Vec3f{1.5, -0.5, -18}, 3, redRubber})
	shapes = append(shapes, Sphere{Vec3f{7, 5, -18}, 4, mirror})
	duck := LoadObj(filepath.Join(rootpath, "resources", "duck.obj"))
	duck.material = mirror
	shapes = append(shapes, duck)
