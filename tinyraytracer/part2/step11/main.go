...
func render(spheres []Sphere, lights []Light) {
	...
	pixmap := make([]color.Color, (width-delta)*height)
	for j := range height {
		for i := range width - delta {
			c1 := framebuffer1[i+delta+j*width]
			c2 := framebuffer2[i+j*width]
			...
			arg1 := (c1[0] + c1[1] + c1[2]) / 3
			arg2 := (c2[0] + c2[1] + c2[2]) / 3

			r := uint8(255 * arg1)
			g := uint8(0)
			b := uint8(255 * arg2)
			pixmap[i+j*(width-delta)] = color.RGBA{r, g, b, 255}
		}
	}

	writePng("out", pixmap, width-delta, height)
}
