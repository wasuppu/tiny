...
func render(spheres []Sphere, lights []Light) {
	...
	framebuffer1 := make([]Vec3f, width*height)
	framebuffer2 := make([]Vec3f, width*height)
	...
	for j := range height {
		for i := range width {
			...
			framebuffer1[i+j*width] = castRay(Vec3f{-eyesep / 2, 0, 0}, dir, spheres, lights, 0)
			framebuffer2[i+j*width] = castRay(Vec3f{+eyesep / 2, 0, 0}, dir, spheres, lights, 0)
		}
	}

	k1, k2 := 0.12, 0.10
	ci := float64(width-delta) / 2
	cj := float64(height) / 2
	R := math.Min(float64(width-delta), float64(height)) / 2

	pixmap := make([]color.Color, (width-delta)*height*2)
	for j := range height {
		for i := range width - delta {
			di, dj := float64(i), float64(j)
			r := math.Sqrt(math.Pow(di-ci, 2)+math.Pow(dj-cj, 2)) / R
			ui := ci + (di-ci)*(1+k1*math.Pow(r, 2)+k2*math.Pow(r, 4))
			uj := cj + (dj-cj)*(1+k1*math.Pow(r, 2)+k2*math.Pow(r, 4))
			var c1, c2 Vec3f
			if ui >= 0 && int(ui) < width-delta && uj >= 0 && int(uj) < height {
				c1 = framebuffer1[int(ui)+delta+int(uj)*width]
				c2 = framebuffer2[int(ui)+int(uj)*width]
			}

			max1 := math.Max(c1[0], math.Max(c1[1], c1[2]))
			if max1 > 1 {
				c1 = c1.Muln(1. / max1)
			}
			max2 := math.Max(c2[0], math.Max(c2[1], c2[2]))
			if max2 > 1 {
				c2 = c2.Muln(1. / max2)
			}

			pixmap[j*(width-delta)*2+i] = color.RGBA{uint8(255 * c1[0]), uint8(255 * c1[1]), uint8(255 * c1[2]), 255}
			pixmap[j*(width-delta)*2+i+width-delta] = color.RGBA{uint8(255 * c2[0]), uint8(255 * c2[1]), uint8(255 * c2[2]), 255}
		}
	}

	writePng("out", pixmap, (width-delta)*2, height)
}
