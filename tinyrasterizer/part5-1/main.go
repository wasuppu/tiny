...
func (iv *ImageView) Draw(viewport Viewport, command DrawCommand) {
	for vertexIndex := uint32(0); vertexIndex+2 < command.mesh.count; vertexIndex += 3 {
		...
		end := ClipTriangle(clippedVertices[:], 0, 3)

		for i := 0; i < end; i += 3 {
			v0 := clippedVertices[i:end][0].position
			v1 := clippedVertices[i:end][1].position
			v2 := clippedVertices[i:end][2].position

			c0 := clippedVertices[i:end][0].color
			c1 := clippedVertices[i:end][1].color
			c2 := clippedVertices[i:end][2].color
...
func main() {
	...
	for running {
		...
		// {
		// 	model := Translate(Vec3f{0, 0, -4}).Mul(RotateZX(tm).Mul(RotateXY(tm * 1.61)))
		// 	projection := Perspective(4, 10, math.Pi/3, float64(width)/float64(height))
