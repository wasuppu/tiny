...
func (iv *ImageView) Draw(command DrawCommand) {
	for vertexIndex := uint32(0); vertexIndex+2 < command.mesh.count; vertexIndex += 3 {
		v0 := command.mesh.positions[vertexIndex+0].AsPoint()
		v1 := command.mesh.positions[vertexIndex+1].AsPoint()
		v2 := command.mesh.positions[vertexIndex+2].AsPoint()

		for y := int32(0); y < iv.height; y++ {
			for x := int32(0); x < iv.width; x++ {
				...
				if det01p >= 0 && det12p >= 0 && det20p >= 0 {
					iv.Set(x, y, command.mesh.color.ToColor())
				}
			}
		}
	}
...
func main() {
	...
	for running {
		...
		colorBuffer.Draw(DrawCommand{Mesh{positions: vertices, color: Vec4f{1, 0, 0, 1}, count: 3}})
