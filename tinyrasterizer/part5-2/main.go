			...
			for y := ymin; y <= ymax; y++ {
				for x := xmin; x <= xmax; x++ {
					...
					if det01p >= 0 && det12p >= 0 && det20p >= 0 {
						...
						if int(math.Floor(c.X()*8)+math.Floor(c.Y()*8))%2 == 0 {
							iv.Set(x, y, color.RGBA{0, 0, 0, 255})
						} else {
							iv.Set(x, y, color.RGBA{255, 255, 255, 255})
						}
					}
				}
			}
...
func main() {
	...
	for running {
		...
		// model := Translate(Vec3f{0, 0, -2})
		model := Translate(Vec3f{0, 0, -2}).Mul(RotateYZ(tm))
		projection := Perspective(0.01, 10, math.Pi/3, float64(width)/float64(height))

		command := DrawCommand{
			mesh:      Mesh{positions, colors, indices, 6},
			cullMode:  None,
