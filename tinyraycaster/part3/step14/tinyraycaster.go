...
func drawSprite(sprite *Sprite, fb *Framebuffer, player *Player, texsprites *Texture) {
	...
	spriteDist := math.Sqrt(math.Pow(player.x-sprite.x, 2) + math.Pow(player.y-sprite.y, 2))                             // distance from the player to the sprite
	spriteScreenSize := int(math.Min(1000, float64(int(float64(fb.h)/spriteDist))))                                      // screen sprite size
	...
	for i := range spriteScreenSize {
		...
		for j := range spriteScreenSize {
			...
			fb.SetPixel(fb.w/2+hoffset+i, voffset+j, color.RGBA{0, 0, 0, 255})
		}
	}
...
func render(fb *Framebuffer, m *Map, player *Player, sprites []Sprite, texwalls *Texture, texmonst *Texture) {
	...
	for i := range len(sprites) {
		mapShowSprite(&sprites[i], fb, m)
		drawSprite(&sprites[i], fb, player, texmonst)
	}
}
...
func main() {
	...
	sprites := []Sprite{{1.834, 8.765, 0}, {5.323, 5.365, 1}, {4.123, 10.265, 1}}
