...
func signedDistance(p Vec3f) float64 {
	s := p.Normalize().Muln(SPHERE_RADIUS)
	displacement := math.Sin(16*s.X()) * math.Sin(16*s.Y()) * math.Sin(16*s.Z()) * NOISE_AMPLITUDE
	...
	for j := range height {
		for i := range width {
			...
			if sphereTrace(Vec3f{0, 0, 3}, dir, &hit) {
				...
				framebuffer[i+j*width] = toColor(Vec3f{1, 1, 1}.Muln(lightIntensity))
			} else {
