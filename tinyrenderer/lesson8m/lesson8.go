...
func (s Shader7) Fragment(glFragCoord Vec3f, bar Vec3f) (color.Color, bool) {
	...
	if shadowbuffer[int(glFragCoord.X()+glFragCoord.Y()*width)]-glFragCoord.Z() < 1e-2 {
...
func main() {
	...
	shadowbuffer := make([]float64, int(width*height))
	...
	for iter := 1; iter <= nrenders; iter++ {
		...
		for i := range isize {
			for j := range isize {
				...
				total.Set(i, j, color.Gray{uint8((t*float64(iter-1)+o)/float64(iter) + 0.5)})
			}
		}
	}
	...
	aoshader := AOShader{}
	aoshader.aoimage.read(filepath.Join(parentpath, "lesson8-occlusion.tga"))
