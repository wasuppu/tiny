...
func processScope(fun *Function, symtable *SymbolTable) {
	...
	fun.deco["local"] = []string{}               // set of local variable names: len*4 is the memory necessary on the stack, the names are here to be put in comments
	...
	for _, v := range fun.vars { // process local variables
		symtable.addVar(v.name, &v.deco)
		fun.deco["local"] = append(fun.deco["local"].([]string), v.name)
	}
...
func processStat(n Statement, symtable *SymbolTable) Statement {
	...
	case Assign:
		...
		e.deco["scope"] = deco["scope"]
		e.deco["offset"] = deco["offset"]
