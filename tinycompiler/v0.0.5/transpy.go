...
func transpy(n Function) string {
	...
	set, ok := n.deco["nonlocal"].(map[string]bool)
	if ok && len(set) > 0 {
		nonlocals = "nonlocal "
		for k := range set {
			nonlocals += k + ", "
		}
		nonlocals = strings.TrimSuffix(nonlocals, ", ")
	} else {
		nonlocals = "pass" // no non-local variables'
	}
	...
	str += indent([]string{nonlocals + "\n", allocvars + "\n", nestedfun + "\n", funbody + "\n"})
	if n.name == "main" && len(n.args) == 0 {
		str += fmt.Sprintf("%s()\n", funname)
	}
	return str
}
...
func expr(n Expression) string {
	...
	case FunCall:
		...
		return fmt.Sprintf("%s(%s)", e.deco["fundeco"].(map[string]any)["label"].(string), args)
	default:
